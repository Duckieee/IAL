...
*/
    //pokud neni fronta prazdna, ulozim pocatecni znak fronty do parametru c
    if (queueEmpty(q) == 1) {
...
*/
    //pokud fronta neni prazdna, vymazu pocatecni znak fronty do parametru c
    if (queueEmpty(q) == 1) {
        queueError(QERR_REMOVE);
        return;
    }

    int nextI = nextIndex(q->f_index);
    q->f_index = nextI;
}
...
*/
    ...
    //pokud fronta neni plna, ulozim znak v parametru c do fronty
    q->arr[q->b_index] = c;
